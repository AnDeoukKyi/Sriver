...
int DB_Insert_ParkingPoint(int index, int occupy, int startX, int startY);
	...
	void UploadDB() {
		...
		for (int i = 0; i < index; i++) {
			if (i == 0) {//0
				...
				DB_Insert_ParkingPoint(i, 0, pList[i]->startX - sX, pList[i]->startY - sY, pList[i]->lenX - sX, pList[i]->lenY - sY);
				sumX += pList[i]->lenX;
				sumY += pList[i]->lenY;
			}
		}
...
void Distortion()
{
	...
	inRange(img_hsv, Scalar(low_hue1, 20, 100), Scalar(high_hue1, 255, 200), img_mask1);
	if (range_count == 2) {
	...
	erode(img_mask1, img_mask1, getStructuringElement(MORPH_ELLIPSE, Size(3, 3)));      //모폴로지침식
	dilate(img_mask1, img_mask1, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)));       //모폴로지팽창
	...
	cout << "======================원본==============================" << endl;
	cout << "왼쪽 위 좌표 : " << Point2f(p_left, p_top) << endl;
	cout << "오른쪽 위 좌표 : " << Point2f(p_left + p_width, p_top) << endl;
	cout << "오른쪽 아래 좌표 : " << Point2f(p_left + p_width, p_top + p_height) << endl;
	cout << "왼쪽 아래 좌표 : " << Point2f(p_left, p_top + p_height) << endl;
	cout << "========================================================" << endl;
	...
	Point2f pts2[4] = {      // 목적 영상 좌표 4개
		...
		Point2f(70, 50),	// 왼쪽 위 (왼쪽 옆, 위)
		Point2f(350, 50),	// 오른쪽 위 (오른쪽 옆, 위)
		Point2f(130, 350),	// 왼쪽 아래 (전체 높이, 사각형 높이 )
...
void Labeling()
{
	for (;;) {
		...
		inRange(warp_img_hsv, Scalar(warp_low_hue1, 20, 100), Scalar(warp_high_hue1, 255, 200), warp_img_mask1);
		...
		erode(warp_img_mask1, warp_img_mask1, getStructuringElement(MORPH_ELLIPSE, Size(15, 15)));          //모폴로지침식
		dilate(warp_img_mask1, warp_img_mask1, getStructuringElement(MORPH_ELLIPSE, Size(3, 3)));         //모폴로지팽창
...
void pand(Mat mat1, Mat mat2) {
	...
	if (trackingEnabled) {
		...
		dilate(thresholdImage, thresholdImage, getStructuringElement(MORPH_ELLIPSE, Size(3, 3)));         //모폴로지팽창
																										  //imshow("모폴로지", warp_img_mask1);
		...
		for (warp_j = 1; warp_j < warp_numOfLables; warp_j++) {
			...
			int y = warp_centroids.at<double>(warp_j, 1);

			circle(mat1, Point(x, y), 5, Scalar(255, 0, 0), 1);
			if (warp_area < 200) continue;
			pos[index][0] = x;
			pos[index++][1] = y;
			if (index == 10) break;
			//cout << "포인트"<<Point(x, y) << endl;
		...
		if (uList.index != 0) {
			if (uList.index == 1) {
				...
				double min = 9999;
				int choice = 0;
				int px = uList.uList[0]->posX;
				int py = uList.uList[0]->posY;

				//가장 가까운 점 탐색
				for (int j = 0; j < 10; j++) {
					if (min > distance(pos[j][0], pos[j][1], px, py)) {
						min = distance(pos[j][0], pos[j][1], px, py);
						choice = j;
					}
				}
				uList.uList[0]->setX(pos[choice][0]);
				uList.uList[0]->setY(pos[choice][1]);
				if (choice == 0) {
					uList.uList[1]->setX(pos[1][0]);
					uList.uList[1]->setY(pos[1][1]);
				}
				else {
					uList.uList[1]->setX(pos[0][0]);
					uList.uList[1]->setY(pos[0][1]);
				}
			}
			for (int i = 0; i<uList.index; i++)
				DB_Update_UserPos(uList.uList[i]->id, uList.uList[i]->posX, uList.uList[i]->posY);
		}
	}
...
void trac() {
	...
	Mat frame, df;
	Mat frameMat1, frameMat2;
	videoCapture.read(frame);
	frameMat1 = VideoDistortion(frame);
	//videoCapture.read(frameMat1);
	...
	for (;;) {
		...
		case 0:
			videoCapture.read(frame);
			frameMat2 = VideoDistortion(frame);
			cas = 1;
...
int DB_Insert_ParkingPoint(int index, int occupy, int startX, int startY, int lenX, int lenY) {
	...
	for (int i = 0; i < 6; i++) {
		itoa(arg[i], tmp, 10);
		sprintf(buf, "%s%s%s%s", argcat[i], tmp);
